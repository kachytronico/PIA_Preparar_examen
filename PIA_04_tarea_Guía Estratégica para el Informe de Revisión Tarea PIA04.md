...
- Funcion de Limpieza Global: Crea una funcion clean(dataset) que incluya imputacion, encoding y escalado. Esto facilita reutilizarla para Train/Val/Test.
...
- Problema 2 (Fallos - Semisupervisado):
	...
	- Bucle Self-training: Entrena con Train -> Predice Desconocidos -> Filtra los de alta confianza (>90%) -> Anade a Train -> Repite.
...
- Error: Usar KFold y separar manualmente validacion.
	- Solucion: Si usas CrossValidation (GridSearch usa CV), no necesitas separar un conjunto de validacion manual, el algoritmo lo hace internamente. Separa solo Test.
...
| Etiquetado Auto (P2) | Bucle while o for. Usar predict_proba > 0.90. | Grafica mostrando como disminuyen los unlabeled en cada iteracion. |
